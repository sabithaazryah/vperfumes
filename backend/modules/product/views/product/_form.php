...
<div class="product-form form-inline">
    ...
    <div class="row">
        ...
        <div class='col-md-4 col-sm-6 col-xs-12 left_padd'>
            ...
            if (!$model->isNewRecord) {
                ...
                $categorys = ArrayHelper::map(Category::find()->where(['main_category' => '7'])->orderBy(['category' => SORT_ASC])->all(), 'id', 'category');
            }
        ...
        <div class='col-md-4 col-sm-6 col-xs-12 left_padd'>
            <?php
            if ($model->isNewRecord) {
                $model->item_ean = $this->context->generateProductEan();
            }
            ?>
    ...
    <div class="row">
        ...
        <div class='col-md-3 col-sm-12 col-xs-12 left_padd'><?php
        ...
        <div class='col-md-9 col-sm-12 col-xs-12 left_padd'>
            <?php
            $related_products = [];
            if (!$model->isNewRecord) {
                if (isset($model->related_product)) {
                    $model->related_product = explode(',', $model->related_product);
                    $related_products = ArrayHelper::map(common\models\Product::find()->where(['id' => $model->related_product])->orderBy(['product_name' => SORT_ASC])->all(), 'id', 'product_name');
                }
            }
...
<div class="row product-gallery">
    <?php
    if (!$model->isNewRecord) {
        $file_path = Yii::$app->basePath . '/../uploads/product/' . $model->id . '/profile/' . $model->canonical_name . '_thumb.' . $model->profile;
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <?php
                $brand_category = \common\models\BrandCategory::find()->where(['status' => 1])->all();
                ?>
                <div class='col-md-12 col-sm-12 col-xs-12 left_padd'>
                    <div class="form-group field-brand-brand required ">
                        <label class="control-label" for="brand-brand" style="margin-bottom:10px">Category</label>
                        <select id="brand-category" class="form-control" name="Brand[category]" aria-required="true" aria-invalid="false">
                            <?php
                            if (!empty($brand_category)) {
                                foreach ($brand_category as $brand_cat) {
                                    ?>
                                    <option value="<?= $brand_cat->id ?>"><?= $brand_cat->category ?></option>
                                    <?php
                                }
                            }
                            ?>
